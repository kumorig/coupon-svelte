...
<script>
  ...
  async function update() {
    ...
    // UCT is used serverside, so we ensure timezone by adding "Z",
...
<div class="grid grid-cols-1 gap-2">
  ...
  <label><input class="px-1 rounded-sm w-full" bind:value={content} placeholder="テキスト" /></label>
  ...
  <label
    >日付まで<input class="px-1 rounded-sm w-full" type="date" bind:value={valid_to} placeholder="日付まで" required />
  </label>
  <label
    >何回使用可能<input
      class="px-1 rounded-sm w-full"
